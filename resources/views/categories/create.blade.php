    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="post" action="{{ route('categorias.store') }}" class="p-6 space-y-6">
                    @csrf
                    ...
                    @if(auth()->user()->isAdmin() && isset($users) && $users->count())
                        <div>
                            ...
                            <select id="user_id" name="user_id"
                                ...
                                @foreach($users as $u)
                                    <option value="{{ $u->id }}" @selected(old('user_id', auth()->id()) == $u->id)>
                                        {{ $u->name }} ({{ $u->email }})
                                    </option>
                                @endforeach
                            </select>
                    ...
                    {{-- kinds va en data-kinds: @json dentro de x-data=" rompe el atributo por las comillas del JSON --}}
                    <div class="space-y-4 rounded-lg border border-gray-100 bg-gray-50 p-4"
                         data-kinds='@json($kindGroups, JSON_HEX_APOS)'
                         x-data="{
                            tipo: @js(old('type', 'ingreso')),
                            presetKind: @js(old('kind', '')),
                            kinds: {},
                            init() {
                                try {
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-600">Las opciones cambian según si es ingreso o gasto. Úsalas para clasificar mejor tus movimientos.</p>
                    ...
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required placeholder="Ej. Nómina quincenal" />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>

                    <div>
                        <x-input-label for="color" value="Color (#RRGGBB, opcional)" />
                        <x-text-input id="color" name="color" type="text" class="mt-1 block w-full" :value="old('color')" placeholder="#4F46E5" maxlength="7" />
